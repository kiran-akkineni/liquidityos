...
from app.db import init_db, get_db, dict_from_row
from app.utils.helpers import make_id, now_iso, json_dumps
from app.services import sellers, buyers, lots, pricing
...
def prod_seed():
    ...
    # Initialize schema (CREATE IF NOT EXISTS — safe to re-run)
    init_db()

    # Check if data already exists
    with get_db() as conn:
        seller_count = conn.execute("SELECT COUNT(*) as c FROM sellers").fetchone()
        count = seller_count["c"] if isinstance(seller_count, dict) else seller_count[0]
    if count > 0:
    ...
    # Seed canonical products (subset for demo)
    ts = now_iso()
    demo_products = [
    ...
    # Create a demo active lot
    lot = lots.create_lot(seller_id, {
        "title": "Mixed Electronics — 100 units, 2 pallets",
        "description": "Demo lot with consumer electronics. Mix of headphones, speakers, and accessories.",
    ...
    print(f"Demo data seeded: seller={seller_id}, buyer={buyer_id}, lot={lot['lot_id']}")
